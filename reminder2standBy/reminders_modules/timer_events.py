...
def put_system_standby():
    """Function that put system into standby"""
    os.system(
...
class InfinityBeep(threading.Thread):
    ...
    def run(self):
        """Function that plays endless beeps
...
class EventEntry():
    ...
    def __init__(
        ...
        sound_path=None, notification_text=None, function2do=None, thread2start=None
    ):
    ...
    def event_reaction(self, time_passed):
        """Fire event and do its job if not fired before and time elapsed to
        fire event
        ...
        if (
            ...
            if self._thread2start_class:
                self._thread2start_obj = self._thread2start_class(self._sound_path)
...
class EventList():
    ...
    def reset_fired(self):
        """Stop job started by every event and reset them - so they can fire
        again"""
        for event in self._events:
            event.reset_fired()
    ...
    def __init__(self, settings_manager, tray):
        ...
        self._events = EventList(
            EventEntry(
                ...
                os.path.join(PROGRAMM_RESOURCE_PATH, 'Exhale-SoundBible.com-1772711989.wav'),
                QtCore.QCoreApplication.translate('MainTimer', 'Where are you, user?')
            ),
            EventEntry(
                self.settings_manager.current_configuration, tray, 'remind1',
                os.path.join(PROGRAMM_RESOURCE_PATH, 'beep-05.wav'),
                QtCore.QCoreApplication.translate('MainTimer', 'Why I am working?')
            ),
            EventEntry(
                self.settings_manager.current_configuration, tray, 'continuously',
                os.path.join(PROGRAMM_RESOURCE_PATH, 'beep-05.wav'), thread2start=InfinityBeep
            ),
            EventEntry(
                self.settings_manager.current_configuration, tray, 'put_stand_by',
...
#         Event listeners part
    ...
    def timer_event(self):
        ...
        self.settings_manager.ui.notActiveOutLb.setText(
            QtCore.QCoreApplication.translate('MainTimer', '{0} min {1} s').format(
                int(total_sec // 60),
                int(total_sec % 60)
            )
        )
